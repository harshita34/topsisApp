...
def topsis(df, weights, criteria, result_file):
    ...
    for i in range(rows):
        df2_s_best.append(
            np.sqrt(np.sum(np.square(df2.iloc[i, :] - df2_ideal_best))))
        df2_s_worst.append(
            np.sqrt(np.sum(np.square(df2.iloc[i, :] - df2_ideal_worst))))
    ...
    df = df.rename(
        columns={'topsis_score': 'Topsis Score', 'topsis_rank': 'Rank'})
...
def sendEmail(email, result_file):
    ...
    sender_email = "hpandey_be20@thapar.edu"  # Enter your address
